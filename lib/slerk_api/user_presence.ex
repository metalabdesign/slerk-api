defmodule SlerkAPI.UserPresence do
  ...
  defp broadcast_status(status, uid, 1) when status == "online",
    do: Endpoint.broadcast("users:presence", "status_updated", %{uid: uid, status: status})

  defp broadcast_status(status, uid, 0) when status == "offline",
    do: Endpoint.broadcast("users:presence", "status_updated", %{uid: uid, status: status})
